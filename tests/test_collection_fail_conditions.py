...
from panopto.errors import SourceAccessBlockedError, SourceUnavailableError
...
def test_collect_for_targets_marks_source_unavailable_as_failure(monkeypatch, tmp_path: Path):
    def _raise_unavailable(**_kwargs):
        raise SourceUnavailableError(platform="twitter", username="aoc")

    monkeypatch.setattr(collection_service, "collect_twitter_posts", _raise_unavailable)
    monkeypatch.setattr(collection_service, "save_posts", lambda posts, db_path: 0)
    monkeypatch.setattr(collection_service, "query_posts", lambda **kwargs: {"count": 0, "posts": []})
    ...
    assert payload["collected"] == 0
    assert payload["per_target"] == [
        {"platform": "twitter", "username": "aoc", "status": "unavailable", "collected": 0}
    ]
    assert payload["errors"]
