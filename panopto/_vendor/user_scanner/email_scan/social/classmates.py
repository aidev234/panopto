...
async def _check(email: str) -> Result:
    ...
    try:
        async with httpx.AsyncClient(timeout=15.0, follow_redirects=True) as client:
            init_response = await client.get(login_url, headers=headers)
            if init_response.status_code == 403:
                return Result.error("Caught by WAF (403) during Handshake")

            csrf_match = re.search(r'name="_csrf" value="([^"]+)"', init_response.text)
            if not csrf_match:
            ...
            body = response.text
            if "invalid registration/password" in body:
                return Result.taken(url=show_url)
            if "did not find an account for the email address" in body:
